...
const FileUploadAndSplit: React.FC = () => {
    ...
    const handleFileChange = async (event: ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            const zip = new JSZip();
            try {
                const zipContents = await zip.loadAsync(file); // 加载 zip 文件
                const fileNames = Object.keys(zipContents.files);
                const firstFileName = fileNames.find(fileName => !zipContents.files[fileName].dir);

                if (firstFileName) {
                    const fileData = zipContents.files[firstFileName];
                    ...
                    for (let i = 0; i < 3; i++) {
                        ...
                        const end = i === 2 ? content.length : start + partSize;
                        newZip.file(`${firstFileName}-part-${i + 1}.txt`, content.slice(start, end));
                    }
                    
                    const blob = await newZip.generateAsync({ type: "blob" });
                    saveAs(blob, `split-${firstFileName}.zip`);
                }
            } catch (error) {
    ...
    return (
      <div>
          ...
          <button onClick={() => {
              ...
              {isProcessing ? 'Processing...' : 'Upload and Process Zip'}
          </button>
      </div>
  );
};